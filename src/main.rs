...
type Permutations<'a> = Vec<(&'a str, &'a str, Range<usize>)>;
...
fn main() {
    ...
    pretty_print_compressed(output);
}

fn print_compressed(permutations: &Permutations) -> String {
    let mut output = Vec::new();
    for (i, permutation) in permutations.iter().enumerate() {
        output.push(&permutation.0[..permutation.2.start]);
        if i == permutations.len() - 1 {
            output.push(&permutation.1);
        }
    }
    return output.join("");
}

fn pretty_print_compressed(permutations: Permutations) {
...
fn compress<'a>(word_list: &'a Vec<&'a str>) -> Permutations<'a> {
    let mut output = Vec::new();
...
fn score_overlaps<'a>(word_list: &'a Vec<&'a str>) -> Permutations {
    let mut permutations = Vec::new();
    for &prefix in word_list.into_iter() {
        for &suffix in word_list.into_iter() {
            if let Some(overlap_range) = find_overlap(prefix, suffix){
...
fn find_overlap(word1: &str, word2: &str) -> Option<Range<usize>> {
    let mut overlap = 0;
